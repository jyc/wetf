export class Packer {
  constructor(options = {}) {
    ...
    this._stringEncoding =
      ["string", "binary"].indexOf(
        options.encoding?.string?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._keyEncoding =
      ["atom", "binary", "string"].indexOf(
        options.encoding?.key?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._safeIntEncoding =
      ["bigint", "float"].indexOf(
        options.encoding?.safeInt?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._safeBigIntEncoding =
      ["number", "bigint"].indexOf(
        options.encoding?.safeBigInt?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._nullEncoding =
      ["atom", "nil"].indexOf(options.encoding?.null?.toLowerCase() ?? "") +
        1 || 1;
    this._bufferEncoding =
      ["binary", "bitbinary", "string"].indexOf(
        options.encoding?.buffer?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._undefinedEncoding =
      ["atom", "null", "ignore"].indexOf(
        options.encoding?.undefined?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._infinityEncoding =
      ["atom", "null", "ignore"].indexOf(
        options.encoding?.infinity?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._nanEncoding =
      ["atom", "null", "ignore"].indexOf(
        options.encoding?.nan?.toLowerCase() ?? "",
      ) + 1 || 1;
    this._arrayEncoding =
      ["list", "improperlist", "tuple"].indexOf(
        options.encoding?.array?.toLowerCase() ?? "",
      ) + 1 || 3;
    ...
    if (
      ...
      this._e = (string, index) =>
        encoder.encodeInto(string, this._u.subarray(index)).written ?? 0;
      this._b = null;
      this._T = 32;
      if (this._compressor === true) {
      ...
      case "bigint": {
        if (obj === 0n) {
          ...
          if (abs < 18446744073709551616n) {
            ...
            this._u[this._i] = 110;
            this._u[this._i + 2] = Number(neg);
            this._v.setBigUint64(
              this._i + 3,
              abs & 18446744073709551615n,
              true,
            );
            ...
            this._v.setBigUint64(
              this._i + 11,
              (abs >> 64n) & 18446744073709551615n,
              true,
            );
            this._v.setBigUint64(
              this._i + 19,
              (abs >> 128n) & 18446744073709551615n,
              true,
            );
            ...
            while (true) {
              ...
              const k1 = k >> 128n;
              const k2 = k & 340282366920938463463374607431768211455n;
              if ((n >>= 256n)) {
                a.push(
                  k2 & 18446744073709551615n,
                  k2 >> 64n,
                  k1 & 18446744073709551615n,
                  k1 >> 64n,
                );
              } else {
    ...
    if (length < this._T) {
      ...
      for (let n = 0; n < length; n++) {
        ...
        if (c < 128) {
          ...
          const c2 =
            65536 + ((c & 1023) << 10) + (string.charCodeAt(++n) & 1023);
